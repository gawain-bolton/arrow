...
// So for example {LogicalType::INT_32, LogicalType::NONE} means
...
// NONE.
//
static const std::set<std::pair<LogicalType::type, LogicalType::type> >
    logical_type_exceptions = {{LogicalType::INT_32, LogicalType::NONE},
                                 {LogicalType::INT_64, LogicalType::NONE},
                                 {LogicalType::INT_32, LogicalType::DECIMAL},
                                 {LogicalType::INT_64, LogicalType::DECIMAL},
                                 {LogicalType::UTF8, LogicalType::NONE}};
...
StreamReader& StreamReader::operator>>(std::chrono::microseconds& v) {
  CheckColumn(Type::INT64, LogicalType::TIMESTAMP_MICROS);
  int64_t tmp;
  Read<Int64Reader>(&tmp);
...
StreamReader& StreamReader::operator>>(float& v) {
  CheckColumn(Type::FLOAT, LogicalType::NONE);
  Read<FloatReader>(&v);
  return *this;
}

StreamReader& StreamReader::operator>>(double& v) {
  CheckColumn(Type::DOUBLE, LogicalType::NONE);
  Read<DoubleReader>(&v);
  return *this;
}

StreamReader& StreamReader::operator>>(char& v) {
  CheckColumn(Type::FIXED_LEN_BYTE_ARRAY, LogicalType::NONE, 1);
...
StreamReader& StreamReader::operator>>(std::string& v) {
  CheckColumn(Type::BYTE_ARRAY, LogicalType::UTF8);
  ByteArray ba;
...
void StreamReader::ReadFixedLength(char* ptr, int len) {
  CheckColumn(Type::FIXED_LEN_BYTE_ARRAY, LogicalType::NONE, len);
  FixedLenByteArray flba;
  Read(&flba);
...
void StreamReader::CheckColumn(Type::type physical_type,
                               LogicalType::type logical_type, int length) {
  if (static_cast<std::size_t>(column_index_) >= nodes_.size()) {
    if (eof_) {
  ...
  if (logical_type != node->logical_type()) {
    ...
    if (logical_type_exceptions.find({logical_type, node->logical_type()}) ==
        logical_type_exceptions.end()) {
      throw ParquetException("Column converted type mismatch.  Column '" + node->name() +
                             "' has converted type '" +
                             LogicalTypeToString(node->logical_type()) + "' not '" +
                             LogicalTypeToString(logical_type) + "'");
    }
  }
